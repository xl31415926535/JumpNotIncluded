...
assert report.count('PASS: ') == 136
assert 'revision: 8' in (project / 'Assets/Resources/GameAssets.asset').read_text()
...
with ZipFile(windows_zip, 'w', ZIP_DEFLATED, compresslevel=6) as archive:
    ...
    archive.writestr('JumpNotIncluded-Windows/README.txt', '''Jump Not Included - v1.10.1
Extract the entire folder, then run JumpNotIncluded.exe.
Choose 1 PLAYER GAME, then START.

v1.10.1: removed invisible Goomba patrol boundaries. The opening Goomba can walk back to spawn and kill idle Small Mario. Goombas turn at real obstacles and fall into pits.
...
Built with Unity 6000.3.24f1. 136 automated Play Mode checks passed.
